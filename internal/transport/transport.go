...
// emits two one-shot prologue records before the loop: an @@i static device/network
// block (the active interface, its link details, the data-partition usage, and the
// resolver) and an @@c capability block (which streaming services are enabled —
// running daemons via pidof, env-gated features via getenv — for the config view).
// It appends
